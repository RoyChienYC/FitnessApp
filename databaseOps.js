...
const checkDB = "select * from ProfileTable where userid = ?";


const insertNamesDB = "insert into ProfileTable (userid, givenName) values (?,?)";
...
const getMostRecentPrevPlannedDB = "SELECT rowIdNum, activity, MAX(date), amount, userid FROM ActivityTable WHERE amount <= 0 and userid = ? and date BETWEEN ? and ? ";
...
async function post_activity(activity) {
  try {
    ...
    // console.log("THIS IS ACTTOLIST: ", act.ActivityToList(activity))
    await db.run(insertDB, act.ActivityToList(activity));
    // await db.run(insertDB, ['swim',1620621600000, -1, '107045659776632087700']);
  } catch (error) {
 ...
 */
async function get_most_recent_planned_activity_in_range(min, max, request) {
  try {
    let results = await db.get(getMostRecentPrevPlannedDB, [request.user.userid, min, max]);
    /* test delete after */
 ...
 * @returns {float} activity.scalar - measure of activity conducted
 */
async function get_most_recent_entry(request) {
  try {
    let result = await db.get(getMostRecentDB, [request.user.userid]);
 ...
 */
async function get_similar_activities_in_range(activityType, min, max, request) {
  try {
    let results = await db.all(getPastWeekByActivityDB, [activityType, request.user.userid, min, max]);
    return results;
  }
  catch (error) {
    console.log(error);
 ...
 * @param {number} max - ms since 1970
 */
async function delete_past_activities_in_range(min, max, request) {
  try {
    await db.run(deletePrevPlannedDB, [request.user.userid, min, max]);
  }
  catch (error) {
    console.log(error);
  }
...
async function insertNames(userid, givenName){
  let checker = await db.get(checkDB,[userid]);
  if (checker != undefined){
    return;
  }
  await db.run(insertNamesDB,[userid, givenName]);
}


async function retrieveName(userid, response){
  let checker = await db.get(checkDB,[userid]);
  if (checker === undefined){
    /* only browser can alert*/
    //alert("There is something wrong here...");
    return;
  }
  console.log(checker.givenName);
  response.json({ givenName: checker.givenName })
}